...
void WebUI_StartUp() {
  ...
  { HeapSelectIram doAllocationsInIRAM;
    ...
    uint16_t tabMono = ESPUI.addControl(ControlType::Tab, "", "Mono");
    uint16_t tabDuo = ESPUI.addControl(ControlType::Tab, "", "Duo");
    uint16_t tabRGB = ESPUI.addControl(ControlType::Tab, "", "RGB");
    uint16_t tabAutoplay = ESPUI.addControl(ControlType::Tab, "", "Auto Play");
    uint16_t tabSpec = ESPUI.addControl(ControlType::Tab, "", "Spec");
    uint16_t tabControl = ESPUI.addControl(ControlType::Tab, "", "Control");
    uint16_t tabSettings = ESPUI.addControl(ControlType::Tab, "", "Settings");
    uint16_t tabAbout = ESPUI.addControl(ControlType::Tab, "", "About");

//    ESPUI.updateVisibility(ESPUI.addControl(Label, "0", "Plug", None), false);
    // COLORS
    static char hexcolor[10];
    sprintf(hexcolor, "#%06X", (unsigned int) lampParam.COLOR0);
    mainColour1 = ESPUI.addControl(Text, "Main Colour", hexcolor, Wetasphalt, tabMono, colorCall, (void*)0);
    ESPUI.setInputType(mainColour1, "color");
    ESPUI.setElementStyle(mainColour1, CSS_COLOR_STYLE);
    //
    mainColour2 = ESPUI.addControl(Text, "Duo Colours", hexcolor, Wetasphalt, tabDuo, colorCall, (void*)0);
    ...
    // SHOW BUTTONS
    auto mono_label_show = ESPUI.addControl(ControlType::Label, "MONO COLOR LED SHOW:", "", Wetasphalt, tabMono);
    ESPUI.setElementStyle(mono_label_show, "background-color: unset;");
    auto duo_label_show = ESPUI.addControl(ControlType::Label, "DUO COLORS LED SHOW:", "", Wetasphalt, tabDuo);
    ESPUI.setElementStyle(duo_label_show, "background-color: unset;");
    auto rgb_label_show = ESPUI.addControl(ControlType::Label, "RAINBOW COLORS LED SHOW:", "", Wetasphalt, tabRGB);
    ESPUI.setElementStyle(rgb_label_show, "background-color: unset;");
    auto spc_label_show = ESPUI.addControl(ControlType::Label, "SPECIAL LED SHOW:", "", Wetasphalt, tabSpec);
    ESPUI.setElementStyle(spc_label_show, "background-color: unset;");

    // BG COLORS
    sprintf(hexcolor, "#%06X", (unsigned int) lampParam.COLOR1);
    bgColor1 = ESPUI.addControl(Text, "BG Colour", hexcolor, Wetasphalt, tabMono, colorCall, (void*)1);
    ESPUI.setInputType(bgColor1, "color");
    ESPUI.setElementStyle(bgColor1, CSS_COLOR_STYLE);
    bgColor2 = ESPUI.addControl(Text, "BG Colour", hexcolor, Wetasphalt, tabDuo, colorCall, (void*)1);
    ...
    for (uint8_t i = 0; i < num_modes; i++) {
      ESPUI.setElementStyle(ESPUI.addControl(ControlType::Button, "", lamp.getModeName(monoModes[i]), Dark, mono_label_show, &monoModeCall, (void*)monoModes[i]), CSS_SHOWBUTTON_UP_STYLE);
    }
    num_modes = sizeof(duoModes);
    for (uint8_t i = 0; i < num_modes; i++) {
      ESPUI.setElementStyle(ESPUI.addControl(ControlType::Button, "", lamp.getModeName(duoModes[i]), Dark, duo_label_show, &duoModeCall, (void*)duoModes[i]), CSS_SHOWBUTTON_UP_STYLE);
    }
    num_modes = sizeof(rgbModes);
    for (uint8_t i = 0; i < num_modes; i++) {
      ESPUI.setElementStyle(ESPUI.addControl(ControlType::Button, "", lamp.getModeName(rgbModes[i]), Dark, rgb_label_show, &rgbModeCall, (void*)rgbModes[i]), CSS_SHOWBUTTON_UP_STYLE);
    }
    num_modes = sizeof(specModes);
    for (uint8_t i = 0; i < num_modes; i++) {
      ESPUI.setElementStyle(ESPUI.addControl(ControlType::Button, "", lamp.getModeName(specModes[i]), Dark, spc_label_show, &specModeCall, (void*)specModes[i]), CSS_SHOWBUTTON_UP_STYLE);
    }
    auto styleBttn = ESPUI.addControl(ControlType::Button, "Very Special Mode", "From UA with Love", Peterriver, tabSpec, &specModeCall, (void*)59);

    ESPUI.setPanelStyle(styleBttn, CSS_VERYSPEC_STYLE);



    /** Tab - AutoPlay - tabAutoplay  */
    auto autoplay_1_panel = ESPUI.addControl(Label, "Start Auto Play One-by-One:", "", Wetasphalt, tabAutoplay);
    ...
    auto autoplay_2_panel = ESPUI.addControl(Label, "Start Auto Play Random Order:", "", Wetasphalt, tabAutoplay);
    ...
    auto autoplayTimeoutSlider = ESPUI.addControl(Slider, "Next Show every _ mins", String(lampParam.PLAYTIME / 60), Wetasphalt, tabAutoplay, autoplayTimeoutSliderCall);
    ...
    auto autoplay_3_panel = ESPUI.addControl(Label, "Stop Auto Play", "", Wetasphalt, tabAutoplay);
    ...
    /** CONTROLS    - tabControl  */
    // Control Panel
    auto control_panel = ESPUI.addControl(Label, "Power:", "", Wetasphalt, tabControl);
    ...
    // Bri slider
    auto briSlider = ESPUI.addControl(Slider, "Brightness", String(lamp.getBrightness()), Wetasphalt, tabControl, briSliderCall);
    ...
    // Speed slider
    auto speedSlider = ESPUI.addControl(Slider, "Speed", String(lamp.getSpeed() / 10), Wetasphalt, tabControl, speedSliderCall);
    ...
    //    ESPUI.addControl(Separator, "WiFi Credentials ", "", None, tabSettings);
    //    auto wifi_web = ESPUI.addControl(Label, "WiFi Credentials (left empty for ignore WiFi)", "", Wetasphalt, tabSettings);
    ...
    // WiFi AP Credentials
    ESPUI.addControl(Separator, "WiFi Access Point Credentials ", "", None, tabSettings);
    auto wifiap_web = ESPUI.addControl(Label, "WiFi Access Point Credentials", "", Wetasphalt, tabSettings);
    ...
    // Soft panel
    ESPUI.addControl(Separator, "Sofware settings:", "", Wetasphalt, tabSettings);
    auto soft_panel = ESPUI.addControl(Label, "Software Settings:", "", Wetasphalt, tabSettings);
    ...
    // Hard panel
    ESPUI.addControl(Separator, "Hardware settings, save to apply", "", None, tabSettings);
    auto hard_panel = ESPUI.addControl(Label, "Hardware Settings:", "", Wetasphalt, tabSettings);
    ...
    ESPUI.addControl(Separator, "Restart / Reset section</br>(be caution)", "", None, tabSettings);
    ESPUI.setElementStyle(ESPUI.addControl(Button, "Restart System", "Restart Now", Dark, tabSettings, &restartCall), CSS_SAVEBUTTON_STYLE);
    resetBttn = ESPUI.addControl(Button, "Reset Settings to default</br>(hold 5+ sec)", "Reset Now", Dark, tabSettings, &resetCall);
    ESPUI.setElementStyle(resetBttn, CSS_SAVEBUTTON_STYLE);

    /**  tabAbout  */
    String aboutStr     = "Version ";
    aboutStr    += STR_VERSION;
    aboutStr    += "<br>";
    aboutStr    += STR_AUTHOR;
    aboutStr    += "<br>";
    aboutStr    += STR_BLD_DATE;
    aboutStr    += "<br>";
    aboutStr    += HTML_GITHUB_REPO;
    aboutStr    += "<br>";
    auto about_label = ESPUI.addControl(Label, "Info:", aboutStr, Wetasphalt, tabAbout);
    ESPUI.setElementStyle(about_label, "background-color: unset;");
    
    ESPUI.setElementStyle(ESPUI.addControl(Label, "2", HTML_ICON, None, about_label), "background-color: unset;");
